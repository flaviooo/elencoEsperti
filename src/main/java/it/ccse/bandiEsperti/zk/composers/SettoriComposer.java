...
public class SettoriComposer extends GenericForwardComposer implements AdministrationComposer {
	...
	public void onClick$bnt_aggiorna(Event event) {
		...
		if(res){
				try {
					Messagebox.show("La modifica effettuata alla Settore avrà impatto su tutti i Curriculum aventi tale riferimento. Si vuole procedere?", "Question", Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION, 
	   ...
	   //primo evento generato al clic su un pulsante di rimozione
		...
		public void onVerificaRimuovi(ForwardEvent event){
			...
			try{
				...
				if(res){
					Messagebox.show("La cancellazione del Settore non può essere effettuata, la voce è presente all'interno di uno o più Curriculum Vitae"); 
					return;		
				}else{
